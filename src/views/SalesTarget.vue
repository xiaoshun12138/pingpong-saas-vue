...
const updateStoreChart = () => {
  ...
  const stores = storeBarData.value.map(d => d.storeName)
  ...
  storeChart.setOption({
    ...
    series: [
      ...
      { name: '已完成', type: 'bar', data: actuals, itemStyle: { color: '#409EFF', borderRadius: [4, 4, 0, 0] } }
    ]
  })
}
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await api.get('/target-dashboard/sales', { params })
    const data = res.data || {}
